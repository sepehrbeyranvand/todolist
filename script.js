...
$(document).ready(function() {
  ...
  $('#taskSend').click(function() {
    ...
    $('.container-fluid').prepend('<div style="background-color:' + theColor + '" class="alert alert-dismissible w-75 d-flex fade show">' +
      '<button class="btn-close" data-bs-dismiss="alert"></button>' +
      '<div class="d-flex flex-column">' +
      '<h3>' + Value + '</h3>' +
      '<p class="text-sm text-muted">' + Activity + '</p>' +
  ...
  function displayStoredAlerts() {
    var storedAlert = localStorage.getItem('alert');
    if (storedAlert) {
      var alertData = JSON.parse(storedAlert);
      $('.container-fluid').prepend('<div style="background-color:' + alertData.color + '" class="alert alert-dismissible w-75 d-flex fade show">' +
        '<button class="btn-close" data-bs-dismiss="alert"></button>' +
        '<div class="d-flex flex-column">' +
